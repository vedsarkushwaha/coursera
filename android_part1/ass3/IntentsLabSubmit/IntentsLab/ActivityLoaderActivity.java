...
public class ActivityLoaderActivity extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.i(TAG, "Getting the value");
		if(data==null) Log.i(TAG, "Data is null");
		else {
			Log.i(TAG, data.getStringExtra("userValue").toString());
			Log.i(TAG, "Value received");
			// If so, update the Textview showing the user-entered text.
			if (requestCode == GET_TEXT_REQUEST_CODE && resultCode == RESULT_OK) {
				mUserTextView.setText(data.getStringExtra("userValue"));
			}
		}
    }
}